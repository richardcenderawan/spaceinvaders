...
//SurfaceView Adalah sebuah class dari turunan class View.
// Class ini menyediakan surface(permukaan) untuk digambar.
// Akses ke surface(permukaan) diberikan oleh interface SurfaceHolder yang bisa diambil menggunakan metode getHolder().
// Surface(permukaan) akan dibuat ketika jendela SurfaceView visible(terlihat). Kita harus mengimplementasikan metode
// surfaceCreated(SurfaceHolder) dan surfaceDestroyed(SurfaceHolder) untuk mengetahui kapan Surface(permukaan)
// itu dibuat(ditampilkan) dan didestroy(disembunyikan).
...
public class GameView extends SurfaceView implements Runnable {
    ...
    public void update() {
        ...
        if (mCounter % 1000 == 0) {
            mMeteors.add(new Meteor(getContext(), mScreenSizeX, mScreenSizeY));
        }
        ...
        if (mCounter % 2000 == 0) {
            mEnemies.add(new Enemy(getContext(), mScreenSizeX, mScreenSizeY));
        }
    ...
    void drawScore() {
        ...
        mCanvas.drawText("Score : " + SCORE, 50, 50, score);
    }
